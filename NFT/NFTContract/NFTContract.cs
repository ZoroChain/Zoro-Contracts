...
namespace NFT_Token
{
    ...
    public class NFTContract : SmartContract
    {
        ...
        public static object Main(string method, object[] args)
        {
            ...
            if (Runtime.Trigger == TriggerType.Application)
            {
                ...
                if (method == "addPoint") //(tokenId)
                {
                    if (!Runtime.CheckWitness(superAdmin)) return false;
                    if (args.Length != 1) return false;
                    return AddInviterMemberPoint((byte[])args[0]);
                }
        ...
        private static bool AddInviterMemberPoint(byte[] tokenId)
        {
        ...
        {
            BigInteger memberPoint = (BigInteger)args[0]; //邀请会员所得贡献值

            BigInteger twoLevelPercent = (BigInteger)args[1]; //二级上线获得贡献值
            BigInteger threeLevelPercent = (BigInteger)args[2]; //三级上线获得贡献值
            byte[] gatherAdress = (byte[])args[3]; //收款地址
        ...
        {
            byte value0 = (byte)(intValue >> 24);
            byte value1 = (byte)(intValue >> 16);
            byte value2 = (byte)(intValue >> 8);
            byte value3 = (byte)intValue;
            byte[] bytes = new byte[] { value0, value1, value2, value3 };
            return bytes;
        }
